/*
    EOTF Boost v8.7.1.2 - 1D APL Lookup + Optional High APL Adaptive Boost
    Calibrated for monitor SAMSUNG OLED G8 G85SB (intended for 3rd gen QD-OLEDS TB400) 
...
float3 ApplyBoostPreserveColorFromPrecomputedParams(float3 color, float sceneLogGain, float4 boostParams)
{
    if (APLInputMode == 1)
    {
        ...
        if (EnableColorPreservingBoostMode)
        {
            ...
            colorScale = SoftLimitBoostScale(colorScale, maxHuePreservingScale, EnableColorPreservingBoostMode);
        }
